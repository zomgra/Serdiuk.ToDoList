...
namespace Serdiuk.ToDoList.Application.Services
{
    public class ToDoItemService : IToDoItemService
    {
        ...
        public async Task<Guid> AddToDoAsync(CreateToDoItemDto todo, Guid userId)
        {
        ...
        public async Task<Guid> DeleteToDoAsync(DeleteToDoItemDto todo, Guid userId)
        {
        ...
        public async Task<IEnumerable<ToDoItem>> GetAll(Guid userId)
        {
            return await _context.ToDoItems.ToListAsync();
        }

        public async Task<ToDoItem> GetById(Guid id, Guid userId)
        {
        ...
        public async Task<IEnumerable<ToDoItem>> GetCompleteToDoAsync(Guid userId)
        {
        ...
        public async Task<IEnumerable<ToDoItem>> GetIncompleteToDoAsync(Guid userId)
        {
            return await _context.ToDoItems.Where(x => x.UserId == userId)
                .Where(x => !x.IsDone)
                .ToListAsync();
        ...
        public async Task<bool> UpdateDoneAsync(UpdateDoneToDoItemDto todo, Guid userId)
        {
            var entity = await _context.ToDoItems.FirstOrDefaultAsync(x=>x.Id==todo.Id);
            if (entity == null)
                throw new TodoNotFoundException("This todo not found, try again");
        ...
        public async Task<bool> UpdateToDoAsync(UpdateToDoItemDto todo, Guid userId)
        {
            var entity = await _context.ToDoItems.FirstOrDefaultAsync(x => x.Id == todo.Id);
            if (entity == null)
                throw new TodoNotFoundException("This todo not found, try again");
